...
def authorize():
    try:
        ...
        flash(
            f"Ошибка при проверке пользователя в базе данных. Прочая ошибка. {err.args[0]}"
        )
        return render_template("login.html")
    # except:
...
def register():
    try:
        ...
        flash(
            f'Имя пользователя "{err.params[0]}" уже занято. Придумайте уникальное имя пользователя.'
        )
